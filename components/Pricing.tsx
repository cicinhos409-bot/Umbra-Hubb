      ...
      <div className="max-w-7xl mx-auto px-4">
        ...
        <div className="grid md:grid-cols-3 gap-8 items-start">
          {PLANS.map((plan, idx) => (
            <div 
              key={idx} 
              className={`relative flex flex-col p-10 rounded-[48px] border transition-all duration-500 hover:translate-y-[-8px] ${
                plan.popular 
            ...
            >
              ...
              <div className="mb-10">
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <h3 className={`text-2xl font-black uppercase tracking-tight ${
                    plan.tier === ToolTier.TURBO ? 'text-brand-pink' : 
              ...
              <div className="flex-1 space-y-5 mb-10">
                {plan.features.map((feature, fIdx) => (
                  <div key={fIdx} className="flex items-start gap-4">
                    <div className={`shrink-0 mt-1 w-4 h-4 rounded-full flex items-center justify-center ${
                      plan.tier === ToolTier.FREE ? 'bg-white/10' :
                      plan.tier === ToolTier.PRO ? 'bg-brand-purple/20' : 'bg-brand-pink/20'
                    }`}>
                      <Check className={`w-3 h-3 ${
                        plan.tier === ToolTier.FREE ? 'text-gray-400' :
              ...
              <button 
                ...
                className={`w-full py-5 rounded-[24px] font-black uppercase tracking-[0.2em] text-xs transition-all transform active:scale-95 shadow-xl ${
                plan.popular 
