...
const UmbraExtrairTool: React.FC = () => {
  ...
  useEffect(() => {
    const saved = sessionStorage.getItem('umbra_supa_key');
    if (saved) setApiKey(saved);
  }, []);

  const saveApiKey = () => {
    if (!apiKey.trim()) return;
    sessionStorage.setItem('umbra_supa_key', apiKey.trim());
    alert('Chave salva na sessão!');
  };
  ...
  const handleExtract = async () => {
    if (!apiKey) return setError('Configure sua Supadata API Key primeiro.');
    ...
    try {
      ...
      if (!res.ok) {
        updateLog('l1', 'fail');
        if (res.status === 401) throw new Error('API Key inválida (Supadata).');
        if (res.status === 402) throw new Error('Créditos esgotados no Supadata.');
        if (res.status === 404) throw new Error('Transcrição não encontrada ou legendas desativadas.');
      ...
      if (!Array.isArray(content) || content.length === 0) throw new Error('Nenhum conteúdo retornado pela API.');
      ...
      setVideoInfo({ id: videoId, title, lang: data.lang || lang || 'auto' });
      updateLog('l3', 'ok');
    } catch (err: any) {
      ...
      <section className="bg-background-mid border border-white/5 rounded-[40px] p-8 shadow-xl space-y-6">
        <div className="flex items-center justify-between px-1">
          <div className="flex items-center gap-3">
            ...
            <h3 className="font-orbitron text-[10px] font-black uppercase tracking-widest text-white">Configuração Supadata</h3>
          </div>
          <a href="https://dash.supadata.ai" target="_blank" className="text-[9px] font-black text-brand-cyan hover:underline uppercase flex items-center gap-1">Criar Chave Grátis <ExternalLink className="w-2.5 h-2.5" /></a>
        </div>
        <div className="flex gap-3">
           <input 
            ...
            placeholder="Cole sua Supadata API Key..." 
            className="flex-1 bg-background-deep border border-white/10 rounded-2xl p-5 text-xs font-space text-brand-cyan focus:border-brand-cyan outline-none"
           />
           <button onClick={saveApiKey} className="px-8 bg-white/5 border border-white/10 rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-white/10 transition-all">Salvar</button>
        </div>
      </section>
